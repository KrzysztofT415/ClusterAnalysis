...
let clusters_result
let group = (arr, chunk) => Array(arr.length / chunk).fill('').map((_, i) => arr.slice(i * chunk, (i + 1) * chunk))
let makeClusters = _ => {
    ...
    let inertia = 0
    for (const cluster of clusters_result.clusters)
        inertia += cluster.means.reduce((sum, val) => sum + val)
...
let reloadWindow = clusters => {
    ...
    for (let i = 1; i <= clusters; ++i)
        buttons.innerHTML += `<button id='cl${i}' disabled>${i}</button>`
    for (let i = 1; i <= clusters; ++i)
        document.getElementById('cl' + i).onclick = () => loadCluster(i)
